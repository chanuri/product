...
class producthandller extends HttpResponse {
	...
	public function insert($jsonstring) {
		...
		if (empty ( $newproductCode )) {
			$statusCode = 404;
			$rawData = array (
					'error' => 'error has occured while generating product code' 
			);
			$this->publishResponse ( $rawData, 'application/json', $statusCode );
		}
	...
	public function Update($jsonstring) {
		...
		if (empty ( $rawData )) {
			$statusCode = 404;
			$rawData = array (
					'error' => 'error has occured while updating product' 
			);
		} else {
			if (! empty ( $input ["productLog"] )) {
				$activityObj = $this->convertToActivityObject ( $input ["productLog"] );
	...
	public function saveToProductActivity($jsonstring) {
		...
		if (empty ( $rawData )) {
			$statusCode = 404;
			$rawData = array (
					'error' => 'error has occured while saving product activity and comment' 
			);
		} else {
			$statusCode = 200;
		}
	...
	public function updateActivity($jsonString) {
		$activityObj = new activityAndComment ();
		$input = json_decode ( $jsonstring, TRUE );
		$activityObj = $this->convertToActivityObject ( $input );
		if (empty ( $activityObj->logID )) {
			$activityObj->logID = "-999";
		}
		$rawData = $this->saveToProductActivitytoOjectStore ( $activityObj );
		
		if (empty ( $rawData )) {
			$statusCode = 404;
			$rawData = array (
					'error' => 'error has occured while updating product activity data' 
			);
		} else {
			$statusCode = 200;
		}
	...
	public function updateLastTransaction($jsonstring) {
		$input = json_decode ( $jsonstring, TRUE );
		$productKey = $input ["productID"];
		var_dump ( $productKey );
		// echo date('Y-m-d H:i:s');
		$client = ObjectStoreClient::WithNamespace ( DuoWorldCommon::GetHost (), $this->dbtablename, securityToken );
		$rawData = $client->get ()->bykey ( $productKey );
		if (empty ( $rawData )) {
			$statusCode = 404;
			$rawData = array (
			...
			// var_dump ( $rawData );
			$rawData->lastTranDate = date ( 'Y-m-d H:i:s' );
			// var_dump ( $rawData );
			// $statusCode = 200;
			// $client = ObjectStoreClient::WithNamespace ( DuoWorldCommon::GetHost (), $this->dbtablename, securityToken );
			// $profile->profileID = $input ["profileID"];
			$rawData2 = $client->store ()->byKeyField ( "productID" )->andStore ( $rawData );
			
			if (empty ( $rawData2 )) {
				$statusCode = 404;
				$rawData = array (
						'error' => 'Updating lastTranDate has failed!' 
				);
			} else {
				$rawData = $rawData2;
				$statusCode = 200;
